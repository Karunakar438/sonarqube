...
public class RootUserOnOrganizationTest {
  ...
  public void system_administrator_is_flagged_as_root_when_he_enables_organization_support() {
    assertThat(tester.wsClient().rootService().search().getRootsList())
  ...
  public void last_root_can_not_be_unset_root() throws SQLException {
    expectBadRequestError(() -> tester.wsClient().rootService().unsetRoot(UserRule.ADMIN_LOGIN));
  }
  ...
  public void root_can_be_set_and_unset_via_web_services() {
    ...
    expectForbiddenError(() -> user1Session.wsClient().rootService().setRoot(user2.getLogin()));
    expectForbiddenError(() -> user1Session.wsClient().rootService().setRoot(user1.getLogin()));
    expectForbiddenError(() -> user1Session.wsClient().rootService().unsetRoot(user1.getLogin()));
    expectForbiddenError(() -> user2Session.wsClient().rootService().unsetRoot(user1.getLogin()));
    expectForbiddenError(() -> user2Session.wsClient().rootService().unsetRoot(user2.getLogin()));
    // admin (the first root) sets root1 as root
    tester.wsClient().rootService().setRoot(user1.getLogin());
    // root1 can set root root2
    user1Session.wsClient().rootService().setRoot(user2.getLogin());
    // root2 can unset root root1
    user2Session.wsClient().rootService().unsetRoot(user1.getLogin());
    // root2 can unset root itself as it's not the last root
    user2Session.wsClient().rootService().unsetRoot(user2.getLogin());
  }
